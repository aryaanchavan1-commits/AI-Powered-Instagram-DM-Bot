...
from aryan_core.message_storage import MessageStorage
...
def show_auth_page():
    ...
    st.markdown("<h1 style='text-align: center;'>🤖 Instagram DM Bot</h1>", unsafe_allow_html=True)
...
def show_admin_tabs():
    """Show admin tabs"""
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Dashboard", "👥 Users", "💬 Messages", "⚙️ System"])
...
def show_user_tabs():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
        "📊 Dashboard", "🔐 Instagram", "👤 Profile", "🤖 Assistant", "⚙️ Settings", "💬 Messages", "📜 History", "🎮 Bot"
    ])
...
def show_admin_dashboard():
    ...
    with col2:
        threads = message_storage.get_all_threads()
        st.metric("Conversations", len(threads))
    
    with col3:
        messages = message_storage.get_all_messages(limit=1000)
...
def show_message_logs():
    """Show message logs (admin only)"""
    messages = message_storage.get_all_messages(limit=100)
    ...
    for msg in messages[:20]:
        with st.expander(f"Thread: {msg.get('thread_id')} - {msg.get('timestamp', 'N/A')}"):
            st.markdown(f"**Sender:** {msg.get('sender_username', msg.get('sender_id', 'Unknown'))}")
            st.markdown(f"**User Message:** {msg.get('user_message', 'N/A')}")
            st.markdown(f"**AI Reply:** {msg.get('ai_reply', 'N/A')}")
...
def show_system_stats():
    ...
    with col2:
        ...
        st.markdown(f"**Total Conversations:** {len(threads)}")
        st.markdown(f"**Total Messages:** {len(messages)}")
        if messages:
            latest = messages[0].get('timestamp', 'N/A')
            st.markdown(f"**Latest Message:** {latest}")
...
def show_dashboard_home():
    ...
    with col3:
        ...
        st.markdown(f"""
        <div style="background-color: #f8f9fa; padding: 1rem; border-radius: 8px; text-align: center;">
            <div style="font-size: 2rem; font-weight: 700; color: #1a1a2e;">{len(threads)}</div>
            <div style="font-size: 0.9rem; color: #6c757d;">Conversations</div>
        </div>
        """, unsafe_allow_html=True)
...
def show_instagram_login():
    ...
    user_data = user_db.get_user(st.session_state.username)
        ...
        if submitted:
            if ig_username and ig_password:
                with st.spinner("Logging in to Instagram..."):
                    ...
                    result = login_handler.login(ig_username, ig_password, verification_code)
...
def show_messages():
    ...
    selected_thread = st.selectbox("Select Conversation", threads)
    
    if selected_thread:
        thread_info = message_storage.get_thread_info(selected_thread)
        messages = thread_info.get("messages", [])
        
        st.markdown(f"### Conversation: {selected_thread}")
        
        for msg in messages[-20:]:
            st.markdown(f"""
            <div style="background-color: #f8f9fa; padding: 1rem; border-radius: 8px; margin-bottom: 0.5rem;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                    <strong>{msg.get('sender_username', 'Unknown')}</strong>
                    <small style="color: #6c757d;">{msg.get('timestamp', 'N/A')}</small>
                </div>
                <div style="margin-bottom: 0.5rem;">
                    <strong>User:</strong> {msg.get('user_message', 'N/A')}
                </div>
                <div>
                    <strong>AI:</strong> {msg.get('ai_reply', 'N/A')}
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def show_history():
    """Show history page"""
    history = user_db.get_user_history(st.session_state.username, limit=100)
                ...
                if response.status_code == 200:
                    ...
                    for thread in threads:
                        ...
                        if items:
                            ...
                            message_storage.save_message(
                                thread_id,
                                sender,
                                text,
                                ai_reply
                            )
                            print(f"💾 Message saved to storage")
