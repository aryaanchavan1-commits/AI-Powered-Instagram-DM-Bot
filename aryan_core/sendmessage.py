...
# Session file for loading device IDs
    ...
    try:
        if os.path.exists(SESSION_FILE):
            with open(SESSION_FILE, 'r') as f:
                ...
                return session_data.get('device_id', f"android-{uuid.uuid4().hex[:16]}")
...
def mesj(
    ...
    # Use consistent device ID from session if available
    if device_id == "android-1":
